...
<div class="main_body">
    ...
    <form method="POST" action="UpdateLogement.php" enctype="multipart/form-data">
        ...
        <input type="hidden" id="IDlogement" name="IDlogement"
               value="<?= isset($logementDetails['IDlogement']) ? $logementDetails['IDlogement'] : ''; ?>" required>

        <!-- Fields prefilled with existing data -->
        <label for="Nom">Nom :</label><br>
        <input type="text" id="Nom" name="Nom" placeholder="Entrer le nom du logement" required
               value="<?= isset($logementDetails['Nom']) ? $logementDetails['Nom'] : ''; ?>"><br>

        <label for="Type">Type :</label><br>
        <select id="Type" name="Type" required>
            <option value="hotel"<?= isset($logementDetails['Type']) && $logementDetails['Type'] == 'hotel' ? ' selected' : ''; ?>>
                Hôtel
            </option>
            <option value="maison d hote"<?= isset($logementDetails['Type']) && $logementDetails['Type'] == 'maison d hote' ? ' selected' : ''; ?>>
                Maison d'hôte
            </option>
            <option value="villa"<?= isset($logementDetails['Type']) && $logementDetails['Type'] == 'villa' ? ' selected' : ''; ?>>
                Villa
            </option>
            <option value="appartement"<?= isset($logementDetails['Type']) && $logementDetails['Type'] == 'appartement' ? ' selected' : ''; ?>>
                Appartement
            </option>
        </select><br>

        <label for="Adresse">Adresse :</label><br>
        <input type="text" id="Adresse" placeholder="Adresse du logement" required name="Adresse"
               value="<?= isset($logementDetails['Adresse']) ? $logementDetails['Adresse'] : ''; ?>"><br>

        <label for="Prix">Prix par nuitée (Dt) :</label><br>
        <input type="number" id="Prix" placeholder="Prix par nuitée" required name="Prix"
               value="<?= isset($logementDetails['Prix']) ? $logementDetails['Prix'] : ''; ?>"><br>

        <label for="Description">Description :</label><br>
        <textarea id="Description" name="Description" required
                  placeholder="Description du logement"><?= isset($logementDetails['Description']) ? $logementDetails['Description'] : ''; ?></textarea><br>

        <label for="Capacite">Capacité :</label><br>
        <input type="number" id="Capacite" name="Capacite" placeholder="Capacité du logement" required
               value="<?= isset($logementDetails['Capacite']) ? $logementDetails['Capacite'] : ''; ?>"><br>

        <label for="Evaluation">Evaluation :</label><br>
        <input type="number" id="Evaluation" name="Evaluation" min="0" max="5" required
               value="<?= isset($logementDetails['Evaluation']) ? $logementDetails['Evaluation'] : ''; ?>"><br>

        <label for="Disponibilite">Disponibilité :</label><br>
        <select id="Disponibilite" name="Disponibilite" required>
            <option value="disponible" <?= isset($logementDetails['Disponibilite']) && $logementDetails['Disponibilite'] == 'disponible' ? ' selected' : ''; ?>>
                Disponible
            </option>
            <option value="non disponible" <?= isset($logementDetails['Disponibilite']) && $logementDetails['Disponibilite'] == 'non disponible' ? ' selected' : ''; ?>>
                Non disponible
            </option>
        </select><br><br>

        <label for="IDvol"></label>
        <input type="hidden" id="IDvol" name="IDvol" placeholder="IDvol" required
               value="<?= isset($logementDetails['IDvol']) ? $logementDetails['IDvol'] : ''; ?>">
