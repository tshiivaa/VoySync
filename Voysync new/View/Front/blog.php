    ...
    <script>
      function gfg(n, id) {
        ...
        for (let i = 0; i < n; i++) {
          if (n == 1) {
            cls = "one";
            rating = 1; // Définition de la valeur de rating à 1
          } else if (n == 2) {
            cls = "two";
            rating = 2; // Définition de la valeur de rating à 2
          } else if (n == 3) {
            cls = "three";
            rating = 3; // Définition de la valeur de rating à 3
          } else if (n == 4) {
            cls = "four";
            rating = 4; // Définition de la valeur de rating à 4
          } else if (n == 5) {
            cls = "five";
            rating = 5; // Définition de la valeur de rating à 5
          }
      ...
      $(document).ready(function () {
        $(".checkbox").click(function () {
          var articleId = $(this).attr('id').split('_')[1]; // Récupérer l'ID de l'article à partir de l'ID de l'élément
          var menu = $(".menu_" + articleId); // Cibler le menu spécifique à cet article
      ...
      // Fonction pour obtenir le contenu de l'article
      function getArticleContent(articleId) {
        var article = {
          TitreArt: document.getElementById("TitreArt_" + articleId).innerText,
          ContenuArt: document.getElementById("ContenuArt_" + articleId).innerText,
          AuteurArt: document.getElementById("AuteurArt_" + articleId).innerText,
          img: document.getElementById("img_" + articleId).getAttribute('src')
        };
      ...
      // Parcourez chaque lien de partage et ajoutez un gestionnaire d'événements au clic
      shareBtns.forEach(function (btn) {
        btn.addEventListener('click', function (event) {
          // Empêchez le comportement par défaut du lien
          event.preventDefault();

          // Récupérez le contenu de l'article correspondant
          var articleId = this.closest('.box').querySelector('input[type="checkbox"]').id.split('_')[1];
