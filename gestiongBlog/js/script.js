document.addEventListener("DOMContentLoaded", function () {
  ...
  const expandingFlexCards = document.querySelectorAll(
  ...
  expandingFlexCards.forEach((expandingFlexCard) => {
    expandingFlexCard.addEventListener("click", function () {
      ...
      expandingFlexCards.forEach((card) => {
        if (card !== this) {
          card.querySelector(".expanding-flex-cards-title").style.display =
            "none";
        }
      });
  ...
  modifierButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      formSection.querySelector("h3").textContent = "Modifier dépense";
      ...
      const category =
        expenseItem.querySelector(".details .category").textContent;
      const date = expenseItem.querySelector(
        ".details p:nth-child(2)"
      ).textContent;
      const currency =
        expenseItem.querySelector(".details .currency").textContent;
      const location =
        expenseItem.querySelector(".details .location").textContent;
      const id = expenseItem.querySelector(
        ".details p:last-child span"
      ).textContent;
